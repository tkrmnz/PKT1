...
//	ft2 = wiringPiI2CSetup(0x12);
  ...
  INP_GPIO(2);  // Always use INP_GPIO(x) before using SET_GPIO_ALT(x,y)
  INP_GPIO(3);  // Always use INP_GPIO(x) before using SET_GPIO_ALT(x,y)
  SET_GPIO_ALT(2, 0);
  SET_GPIO_ALT(3, 0);
...
//	pinMode (8, INPUT) ; 	// Pin 2 SDA
  ...
  INP_GPIO(2);  // Always use INP_GPIO(x) before using SET_GPIO_ALT(x,y)
  INP_GPIO(3);  // GPIO3 input
  OUT_GPIO(3);  // GPIO3 output
  GPIO_CLR = 1<<3; //GPIO3 = L

	usleep(1300); //needs to wait 1300us for the target to arm.
//	while(1) printf("%x ",digitalRead(8));
	printf("%x\n\r",digitalRead(8));

	while(!digitalRead(8)); //wait for scenario to complete
	printf("Game Complete\n\r");

  GPIO_SET = 1<<3; //GPIO3 = L
	printf("Query Reaction Time\n\r");
...
//	ft2 = wiringPiI2CSetup(0x12);
//	wiringPiI2CReadReg8(ft2,&i2cdata[0]);
//	wiringPiI2CReadReg8(ft2,&i2cdata[1]);
//	wiringPiI2CReadReg8(ft2,&i2cdata[2]);
//	wiringPiI2CReadReg8(ft2,&i2cdata[3]);

	read(ft2,&i2cdata,4);
  printf("%x %x %x %x \n\r",i2cdata[0],i2cdata[1],i2cdata[2],i2cdata[3]);
  printf("%d %d \n\r",((i2cdata[0]<<8)|i2cdata[1]),((i2cdata[2]<<8)|i2cdata[3]));
